...
int ft_verti_2(int **table, int *perimeter, int x)
{
    ...
    {
        if ((table[i][x] != 0) && (zero == 0) && (table[i][x] > temp))
        {
            temp = table[i][x];
            look++;
        }
    ...
    if (((zero != 0) && (perimeter[x + g_lensquare] >= look)) ||
        ((zero == 0) && (perimeter[x + g_lensquare] == look)))
        return (1);
    return (0);
}
...
int ft_verti(int **table, int *perimeter, int x)
{
    ...
    if (((zero != 0) && (look <= perimeter[x]) && (ft_verti_2(table, perimeter, x) != 0))
        || ((zero == 0) && (look == perimeter[x]) && (ft_verti_2(table, perimeter, x) != 0)))
            return (1);
    return (0);
}
